...
public abstract class TypedAugmentModule<TIn, TOut> : IOrchestrationAugmentModule
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var payload in batch.Payloads)
        {
            ...
            var json = JsonSerializer.SerializeToUtf8Bytes(output, SerializerOptions);

            results.Add(new IntegrationPayload(name, BinaryData.FromBytes(json), "application/json", metadata));
        }
    ...
    private TIn Deserialize(IntegrationPayload payload)
    {
        try
        {
            return JsonSerializer.Deserialize<TIn>(payload.GetText(), SerializerOptions)
                ?? throw new InvalidOperationException(
                    $"Module '{Descriptor.Id}' deserialized payload '{payload.Name}' as null; a non-null {typeof(TIn).Name} is required.");
        }
        catch (JsonException exception)
        {
            throw new InvalidOperationException(
                $"Module '{Descriptor.Id}' requires payloads shaped like {typeof(TIn).Name}. Payload '{payload.Name}' failed to deserialize: {exception.Message}",
                exception);
        }
    }
}
